        ...
        switch (logMessageType) {
            case "log:thread-admins": {
                if (logMessageData.ADMIN_EVENT == "add_admin") {
                    ...
                    if (global.configModule[this.config.name].sendNoti) api.sendMessage(`==== 𝗡𝗢𝗧𝗜𝗖𝗘 ==== \nUpdate user ${await Users.getNameUser(logMessageData.TARGET_ID)} to become a successful group administrator`, threadID, async (error, info) => {
                    ...
                    if (global.configModule[this.config.name].sendNoti) api.sendMessage(`==== 𝗡𝗢𝗧𝗜𝗖𝗘 ==== \nUpdate user ${await Users.getNameUser(logMessageData.TARGET_ID)} become a successful member`, threadID, async (error, info) => {
                        if (global.configModule[this.config.name].autoUnsend) {
                            await new Promise(resolve => setTimeout(resolve, global.configModule[this.config.name].timeToUnsend * 1000));
                            return api.unsendMessage(info.messageID);
                        } else return;
            ...
            case "log:thread-icon": {
                ...
                if (global.configModule[this.config.name].sendNoti) api.sendMessage(`» [ 𝗚𝗥𝗢𝗨𝗣 𝗨𝗣𝗗𝗔𝗧𝗘 ] y.replace("emoji", "icon")}\n» Original icon: ${preIcon[threadID] || "unknown"}`, threadID, async (error, info) => {
            ...
            case "log:user-nickname": {
                ...
                if (global.configModule[this.config.name].sendNoti) api.sendMessage(`==== 𝗡𝗢𝗧𝗜𝗖𝗘 ==== \nUpdate user nicknames: ${await Users.getNameUser(logMessageData.participant_id)} to: ${(logMessageData.nickname.length == 0) ? "original name": logMessageData.nickname}`, threadID, async (error, info) => {
                    if (global.configModule[this.config.name].autoUnsend) {
                        await new Promise(resolve => setTimeout(resolve, global.configModule[this.config.name].timeToUnsend * 1000));
                        return api.unsendMessage(info.messageID);
                    } else return;
