...
module.exports.run = async function({
    ...
    try {
        if (event.type !== "message_reply") return api.sendMessage("You have to take a picture.", event.threadID, event.messageID);
        if (!event.messageReply.attachments || event.messageReply.attachments.length == 0) return api.sendMessage("You have to take a picture.", event.threadID, event.messageID);
        ...
        console.log(e)
        return api.sendMessage(`has a harness`, event.threadID, event.messageID);
  }
};