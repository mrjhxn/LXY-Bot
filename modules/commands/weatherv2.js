...
module.exports.run = async function ({
    ...
    if (!area) return api.sendMessage(`Please enter location!`, event.threadID, event.messageID)
    ...
    try {
        const response = (await axios.get(`https://api.accuweather.com/locations/v1/cities/search.json?q=${encodeURIComponent(area)}&apikey=${apikey}&language=en-vn`)).data;
    ...
    function formatHours(hours) { return moment(hours).tz("Asia/Manila").format("HH[h]mm[p]"); }
    const dataWeatherDaily = dataWeather.DailyForecasts;
    const dataWeatherToday = dataWeatherDaily[0];
    let msg = `Today's weather:\n${dataWeather.Headline.Text}` +
        ...
        `\n🏙️ Moon setting: ${formatHours(dataWeatherToday.Moon.Set)}` +
