...
module.exports.run = async function ({ event, args, api }) {
  ...
  if (!nam1) return api.sendMessage("Invalid date of birth!",event.threadID,event.messageID);
  const moment = require("moment-timezone");
  var hientai = moment.tz("Asia/Manila").format("DD/MM/YYYY HH:mm:ss");
  var concac = `${hientai}`;
  ...
  // Nỗ não :>
  return api.sendMessage(`-Date of birth: ${input}\n\n-Number of years passed: ${nam} years \n-Number of months passed: ${thang} months \n-Number of weeks passed: ${tuan} weeks \n-Number of days passed: ${ngay} days \n-Number of hours elapsed: ${gio} hours \n-Number of minutes elapsed: ${phut} minutes \n-Number of seconds elapsed: ${giay} seconds `,event.threadID,event.messageID);
}