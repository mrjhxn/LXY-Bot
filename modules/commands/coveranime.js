module.exports.config = {
    ...
    , description: "Create multi-style banners"
    , commandCategory: "Create an image"
    , usages: ""
    , cooldowns: 5
};
...
module.exports.run = async function ({
    ...
    const lengthchar = (await axios.get('https://raidenapi.richardretada.repl.co/taoanhdep/data2')).data
    if (args[0] == 'list') {
        try {
            var page = 1;
            ...
            msg += `--- Page (${page}/${numPage}) ---\n📌 Use ${global.config.prefix}coveralime list [page number] if you want to move pages`;
    ...
    if (!args[0]) {
        const abcxyz = [
"https://i.postimg.cc/tCKJJQNw/1.png",
"https://i.postimg.cc/bvTwmGff/2.png",
"https://i.postimg.cc/SxcszTZs/3.png",
"https://i.postimg.cc/qqgqhpYV/4.png",
"https://i.postimg.cc/ZqZqP8gL/5.png",
"https://i.postimg.cc/NFVMRWqy/6.png",
"https://i.postimg.cc/WbFtsbg0/7.png",
"https://i.postimg.cc/qBZzfChk/8.png",
"https://i.postimg.cc/1t3529yn/9.png"]
        let o = [];
        ...
        const msg = ({
            body: `[ 𝗖𝗢𝗩𝗘𝗥 𝗔𝗡𝗜𝗠𝗘 ]\n\nReply this message and choose the style you like to create the image \nCan use:\n${global.config.prefix}coveralime list ( character ID list)\n${global.config.prefix}coveralime find [ id ] ( preview of character's photo )`
            , attachment: o
        })
        return api.sendMessage(msg, event.threadID, (err, info) => {
            return global.client.handleReply.push({
...
module.exports.handleReply = async function ({
  ...
  var nameSender = (await Users.getData(event.senderID)).name
   ...
   const res = await axios.get(`https://raidenapi.richardretada.repl.co/poem/chamngon`);
  var poem = res.data.data;
    try {
        if (handleReply.author != event.senderID) return api.sendMessage('List', event.threaID);
        ...
        const lengthchar = (await axios.get('https://raidenapi.richardretada.repl.co/taoanhdep/data2'))
            .data
        ...
        if (handleReply.step == 1) {
            if (isNaN(event.body)) return api.sendMessage('Isnt that a number ?', event.threadID)
            api.unsendMessage(handleReply.messageID);
            return api.sendMessage(`You just selected ${event.body} to reply to this message and entered the character id`, threadID, function (err, info) {
                return global.client.handleReply.push({
                    step: 2
                    , name: "coveralime"
            ...
            api.unsendMessage(handleReply.messageID);
            return api.sendMessage(`You just selected the character as ${event.body} reply to this message and entered the main name`, threadID, function (err, info) {
                return global.client.handleReply.push({
                    step: 3
                    , name: "coveralime"
                    , author: handleReply.author
            ...
            if (handleReply.kieu == 9 && handleReply.step == 3) {
                return api.sendMessage(`You just selected the main name ${event.body}, reply to this message and choose a color (please write no if you want to use the traditional color)`, threadID, function (err, info) {
                    return global.client.handleReply.push({
                        step: 5
                        , name: "coveralime"
                        , author: handleReply.author
            ...
            if (handleReply.kieu == 8 && handleReply.step == 3) {
                return api.sendMessage(`You just selected the main name ${event.body}, reply to this message and enter your profile`, threadID, function (err, info) {
                    return global.client.handleReply.push({
                        step: 3.4
                        , name: "coveralime"
                        , author: handleReply.author
            ...
            if (handleReply.kieu == 7 && handleReply.step == 3) {
                return api.sendMessage(`You just selected the main name ${event.body}, reply to this message and create the blur ratio of the image`, threadID, function (err, info) {
                    return global.client.handleReply.push({
                        step: 3.1
                        , name: "coveralime"
                        , author: handleReply.author
            ...
            if (handleReply.kieu == 5 && handleReply.step == 3) {
                ...
                return api.sendMessage(`You just selected the main name is ${event.body} reply to this message and entered a signature (please enter no font error).`, threadID, function (err, info) {
                    return global.client.handleReply.push({
                        step: 4
                        , name: "coveralime"
                        , author: handleReply.author
        ...
        if (handleReply.step == 3.4) {
            api.unsendMessage(handleReply.messageID)
            return api.sendMessage(`You just entered your profile as ${event.body}, reply to this message and enter the facebook link`, threadID, function (err, info) {
                return global.client.handleReply.push({
                    step: 3.5
                    , name: "coveralime"
                    , author: handleReply.author
        ...
        if (handleReply.step == 3.5) {
            api.unsendMessage(handleReply.messageID)
            return api.sendMessage(`You have just entered the facebook link as ${event.body}, reply to this message and select the color (please write no if you want to use the traditional color)`, threadID, function (err, info) {
                return global.client.handleReply.push({
                    step: 5
                    , name: "coveralime"
                    , author: handleReply.author
        ...
        if (handleReply.step == 3.1) {
            api.unsendMessage(handleReply.messageID)
            return api.sendMessage(`You've just selected the blur ratio of the image as ${event.body}%, reply to this message and select a color (please write no if you want to use a traditional color).`, threadID, function (err, info) {
                return global.client.handleReply.push({
                    step: 5
                    , name: "coveralime"
                    , author: handleReply.author
            ...
            if (handleReply.kieu == 6) {
                return api.sendMessage(`You just selected the subname ${event.body}, reply to this message and enter a profile`, threadID, function (err, info) {
                    return global.client.handleReply.push({
                        step: 4.1
                        , name: "coveralime"
                        , author: handleReply.author
                ...
                return api.sendMessage(`You've just chosen a signature of ${event.body}, reply to this message, and choose a color (write no if you want to use the traditional color).`, threadID, function (err, info) {
                    return global.client.handleReply.push({
                        step: 5
                        , name: "coveralime"
                        , author: handleReply.author
            ...
            api.unsendMessage(handleReply.messageID);
            return api.sendMessage(`You just selected the profile as ${event.body}, reply to this message and enter the date of birth`, threadID, function (err, info) {
                return global.client.handleReply.push({
                    step: 4.2
                    , name: "coveralime"
                    , author: handleReply.author
            ...
            api.unsendMessage(handleReply.messageID);
            return api.sendMessage(`You just entered the date of birth as ${event.body}, reply to this message and select a color (please write no if you want to use the traditional color)`, threadID, function (err, info) {
                return global.client.handleReply.push({
                    step: 5
                    , name: "coveralime"
                    , author: handleReply.author
            ...
            if (type == 1) {
                ...
                return api.sendMessage({
                        body: `Create a Successful 🌸 Image \n\n👑 Photo of ${nameSender} \n⚜️ Image type: ${type}\n👤 Character Name: ${lengthchar[char].name}\n📝 Character ID: ${char}\n🖤 Main name: ${tenchinh}\n🤍 Signature: ${subname}\n️🎨 Image color: ${color}`
            ...
            if (handleReply.kieu == 8) {
                ...
                fs.writeFileSync(pathImg, imageBuffer);
                return api.sendMessage({
                        body: `🌸 Create a Successful 🌸 Image \n\n Image of ${nameSender} n\n👑⚜️ Image type: ${type}\n👤 Character Name: ${lengthchar[char].name}\n📝 Character ID: ${char}\n🖤 Main name: ${tenchinh}\n💬 Biography: ${xam1}\n🔗 Link: ${xam2}\ n️🎨 Image color: ${color}`
                        , attachment: fs.createReadStream(pathImg)
                    }
                    , event.threadID
                    , () => fs.unlinkSync(pathImg)
