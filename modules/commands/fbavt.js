...
if (!args[0]) return api.sendMessage(`» » » FB-AVATAR « « « \n\n${prefix}fbavt id [id to get] <get image by person uid>\n\n${prefix}fbavt link [link to get] <get by that person's link>\n\n${prefix}fbavt user <get the user's own avatar>\n\n${prefix}fbavt user [@mentions] <get avatar the person tagged>`,event.threadID,event.messageID);
else if (args[0] == "id") {
	try {
	var id = args[1];
...
return request(encodeURI(`https://graph.facebook.com/${id}/picture?height=720&width=720&access_token=6628568379%7Cc1e620fa708a1d5696fb991c1bde5662`)).pipe(fs.createWriteStream(__dirname+'/cache/1.png')).on('close',() => callback());
    ...
    return request(encodeURI(`https://graph.facebook.com/${mentions}/picture?height=720&width=720&access_token=6628568379%7Cc1e620fa708a1d5696fb991c1bde5662`)).pipe(fs.createWriteStream(__dirname+'/cache/1.png')).on('close',() => callback());
		...
		api.sendMessage(`Wrong order. Type ${prefix} to view module commands.`,event.threadID,event.messageID);
	}
}
else {
	api.sendMessage(`Wrong order. Type ${prefix} to view module commands.`,event.threadID,event.messageID);
