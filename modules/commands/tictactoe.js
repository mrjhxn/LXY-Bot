...
function move(x, y, data) {
  ...
  } else return "Unavailable Position!";
...
module.exports.handleReply = async function({ event, api, handleReply }) {
  ...
  if(!isNaN(number) && number > 0 && number < 10) {
    ...
    if(checkGameOver(data)) {
      var gayban = ["easy game!","Ez!"];
      if(checkAIWon(data)) lmao = `You lose! ${gayban[Math.floor(Math.random() * gayban.length)]} \ndecrease 500$ to your coins`;
      if(checkAIWon(data)) await Currencies.decreaseMoney(senderID,500);
      else if(checkPlayerWon(data)) lmao = "You win! :< \n25k coins added";
      else if(checkPlayerWon(data)) await Currencies.increaseMoney(senderID,25000);
      else lmao = "Draw!! HAHAHAH";
      global.moduleData.tictactoe.delete(threadID);
    }
    var msg = lmao !== "" ? lmao : temp == undefined ? "Reply a number to choose!" : temp;
  ...
  } else return api.sendMessage("Choose correct number!", threadID, messageID);
}
...
module.exports.run = async function ({ event, api, args }) {
  ...
  let newData;
  if (args.length == 0) return api.sendMessage("Please choose X or O", threadID, messageID);
  if (args[0].toLowerCase() == "delete") {
    global.moduleData.tictactoe.delete(threadID);
    return api.sendMessage("delete!", threadID, messageID);
  }
  if (args[0].toLowerCase() == "continue") {
    if (!data.gameOn) return api.sendMessage("continue " + concak + " x/o choose and reply", threadID, messageID);
    return api.sendMessage({ body: "Choose a number and reply!", attachment: await displayBoard(data)}, threadID, (error, info) => {
      global.client.handleReply.push({
        name: this.config.name,
        author: senderID,
        messageID: info.messageID
  ...
  if (!data.gameOn) {
    ...
    if (abc !== "x" && abc !== "o") return api.sendMessage("Please choose X or O", threadID, messageID);
    if (abc == "o") {
      newData = startBoard({ isX: false, data, threadID });
      api.sendMessage({ body: "Choose a number and reply!", attachment: await displayBoard(newData)}, threadID, (error, info) => {
        global.client.handleReply.push({
          name: this.config.name,
          author: senderID,
          messageID: info.messageID
    ...
    if (abc == "x") {
      ...
      AIStart(newData);
      api.sendMessage({ body: "Choose a number and reply!", attachment: await displayBoard(data)}, threadID,(error, info) => {
        global.client.handleReply.push({
          name: this.config.name,
          author: senderID,
          messageID: info.messageID
  ...
  } else return api.sendMessage("Want to play again?\n" + concak + " continue -> to continue" + concak + " delete -> delete saves", threadID, messageID);
  }