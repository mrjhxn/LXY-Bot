...
module.exports.run = ({ event, api, args, permssion }) => {
    ...
    switch (args[0]) {
        case "add": {
            if (permssion == 0) return api.sendMessage("[Note] You do not have enough permission to be able to use more notes with only administrators to be used!", threadID, messageID);
            ...
            api.sendMessage('[Note] added new notes to the group successfully!', threadID, messageID);
            break;
        }
        case "list":
        case"all": {
            ...
            if (msg.length == 0) return api.sendMessage("[Note] Your group does not have a list of notes to display!", threadID, messageID);
