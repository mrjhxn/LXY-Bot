module.exports.config = {
  ...
  module.exports.run = async function({ api, args, event }) {
	...
	if(args[0] == "list") {
		...
		const res = await axios.get("https://manhict.tech/list");
		...
		msg += `» Have everything ${danhsach} character\n» Number of pages (${trang}/${soTrang})\n» Use ${global.config.PREFIX}fbcoverv2 list <number of pages> to be able to see the next page`;
	  return api.sendMessage(` ─ Emilia ── \n` + msg + `\n ── End ── `, threadID, messageID);
	  } else if(args[0] == "find"){
	 var char = args[1];
	  
	  const res = await axios.get(`https://www.nguyenmanh.name.vn/api/searchAvt?key=${encodeURIComponent(char)}`);
  ...
  module.exports.handleReply = async function({ api, event, args, handleReply, client, __GLOBAL, Threads, Users, Currencies }) {
	...
	switch (handleReply.type) {
	  case "characters": {
		...
		const res = await axios.get(`https://manhict.tech/taoanhdep/search/id?id=${id + 1}`);
	  ...
	  case "create": {
		...
		return api.sendMessage(`Processing...`, event.threadID, async (error, info) => {
		  ...
		  var imag = (await axios.get(`https://manhict.tech/fbcover/v2?name=${encodeURIComponent(name_)}&id=${idchar}&subname=${encodeURIComponent(subname_)}&color=${encodeURIComponent(color_)}&apikey=mzk_4iK3OY`, {
			responseType: "stream"
		  })).data;
		  var msg = {
