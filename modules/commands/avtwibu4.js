...
module.exports.run = async ({ api, event, args }) => {
    ...
    if (!text[0]) return api.sendMessage(`Just add text\n\n${prefix}avtwibu4 ID-NUMBER (1-820) | TEXT | TEXT`, event.threadID, event.messageID);
    else{
    var lstct = ["Hs2lvnqu",
                 "DFFAzoK9",
                 "MKo2jiZV",
    ...
    var apikey = lstct[Math.floor(Math.random() * lstct.length)]
            ...
            let getAvatar = (await axios.get(`https://www.nguyenmanh.name.vn/api/avtWibu4?id=${text[0]}&tenchinh=${text[1]}&tenphu=${text[2]}&apikey=${apikey}`, { responseType: "arraybuffer" } )).data; 
