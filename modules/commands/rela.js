...
description: "1: Use command + tag\n2: Use command + info or fake\n\nInfo used to view information like credit\nFake used to create fake banner information",
...
var data = [
  ...
  "a bit low but that's okay. Try!",
  "3 parts of predestined relationships, 7 parts of efforts",
  ...
  "Save each other's numbers, when you get married, call each other up the aisle!",
...
if(!mentions1) { 
  ...
  if(args[0] == "info"){return api.sendMessage(`©Code By DVB Developer\n©Design By DVB Design\n\n=============\n- Support code: Nguyen Thai Hao\n- Idea: Le Dinh\n\ n=============\nIf you want to suggest features, please inbox`,event.threadID,event.messageID)}
  else{return api.sendMessage(`1: Use command + tag\n2: Use command + info or fake\n\nInfo used to view information like credit\nFake used to create fake banner information`,event.threadID,event.messageID)};
};
...
//Chạy handle reply
return api.sendMessage(`enter your tym number for example 8|8|8|8|8`, event.threadID, (err, info) => {
...
module.exports.handleReply = async function({ api, event, args, handleReply, client, __GLOBAL, Threads, Users, Currencies }) {
    ...
    //Hàm soát mảng
    if(!MissionC.length == "5"){return api.sendMessage(`Missing number or wrong format`, threadID, messageID)}
     
    //Hàm tính tổng
    var allmath = (parseInt(MissionC[0])+parseInt(MissionC[1])+parseInt(MissionC[2])+parseInt(MissionC[3])+parseInt(MissionC[4])) * 2.5;
    }catch(e){
      api.sendMessage(`Missing or wrong format \n sorry: ${e}`, threadID, messageID);
    };
    ...
    // Bí mật =))
    if ((this.config.credits) != "DVB Developer") { return api.sendMessage(`Anything for Bang, please give me the credit :3`, event.threadID, event.messageID)}
}
}
};
