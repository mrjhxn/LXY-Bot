module.exports.config = {
	...
	usages: "[first select the number from 1-100/ then press KenDu then press the desired nickname]",
	cooldowns: 5
};
	...
	switch (handleReply.step) {
		...
		case 3:
			...
			let res = await axios.get(encodeURI(`https://api-ttk.herokuapp.com/canvas/fbcover?name=${c.ky}&id=${c.nhanvat}&subname=${c.nen}`), { responseType: "arraybuffer" })
				.catch(e => { return send("Unknown error, contact admin to fix") });
			if (res.status != 200) return send("An error occurred, please try again later!");
