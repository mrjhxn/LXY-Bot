...
var lstct = ["Hs2lvnqu",
                 ...
                 "IyWIYp8J",
                 "tvXOET6H"];
    var keyAPI = lstct[Math.floor(Math.random() * lstct.length)];
...
const UID = sid
  switch (args[1]) {
    case 'fb1': {
      const nam = new DC_Nam('canvas', sid, 'fb1', 'record', new Array());
       nam.create();
    api.sendMessage('» Reply to this message to enter "first name" to activate\n\nif the bot not reply do it until reply just type\n\n!canvas fb1 and send\n!canvas fb1 and send\n\nafter that second chat just reply in second chat', tid, (e, i) => {
  ...
  case 'avtwibu1': {
    const nam = new DC_Nam('canvas', sid, 'avtwibu1', 'record', new Array());
      nam.create();
    api.sendMessage('» Reply to this message to enter "Character ID (1-820 .))"("no" => random)', tid, (e, i) => {
    global.client.handleReply.push({
        'name': config.name,
        'messageID': i.messageID,
        'author': sid,
    ...
    const Nam = new DC_Nam('canvas', sid, 'setstepthuphap', 'step', 3);
      Nam.create();
    api.sendMessage('» Reply to this message to enter "ID Banner(1-3)"("no" => random)', tid, (e, i) => {
    global.client.handleReply.push({
        'name': config.name,
        'messageID': i.messageID,
        'author': sid,
    ...
    break;
      ...
      case 'tiki': {
    api.sendMessage('» Reply to this message to enter "Content"', tid, (e, i) => {
    global.client.handleReply.push({
        'name': config.name,
        'messageID': i.messageID,
        'author': sid,
  ...
  api.sendMessage(list, tid, mid);
    ...
    break;
      ...
      case 'findanime': {
    api.sendMessage('» Reply to this message to enter "waifu name or ID(1-820)"', tid, (e, i) => {
    global.client.handleReply.push({
        'name': config.name,
        'messageID': i.messageID,
        'author': sid,
...
'https://i.postimg.cc/d02nPbSb/image.png'];
      ...
      + `${pn} listanime\n`
      + `${pn} findanime\n\n`
      + `» Call the command as above (k reply this message)`
...
const gCanvas = await global.canvas[sid];
  switch (_.case) {
    case 'fb1': {
      ...
      if (_.step < 5) {
      const $_ = ['"Full Name"', '"Current city"', '"Gmail"', '"Contact Number"', '"Color (English)"']
   api.sendMessage('» Reply to this message to enter ' + $_[_.step], tid, async (e, i) => {
    global.client.handleReply.push({
        'name': config.name,
        'messageID': i.messageID,
        'author': _.author,
    ...
    case 'avtwibu1': {
      ...
      if (_.step < 2) {
      const $_ = ['"Text background"', '"Signature"']
   api.sendMessage('» Reply to this message to enter ' + $_[_.step], tid, async (e, i) => {
    global.client.handleReply.push({
        'name': config.name,
        'messageID': i.messageID,
     ...
     try {
      ...
      if (_.step < 3) {
      const $_ = ['"User Facebook"', '"Primary name"', '"Sub name"']
   api.sendMessage('» Reply to this message to enter ' + $_[_.step], tid, async (e, i) => {
    global.client.handleReply.push({
        'name': config.name,
        'messageID': i.messageID,
     ...
     try {
      ...
      const $_ = ['"Number of lines of text"', '"Line 1"', '"Line 2"', '"Line 3"']
   api.sendMessage('» Reply to this message to enter ' + $_[_.step], tid, async (e, i) => {
    global.client.handleReply.push({
        'name': config.name,
        'messageID': i.messageID,
