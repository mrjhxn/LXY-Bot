...
try { 
    if (appState) {
        ...
        let rl = readline.createInterface({
         ...
         const localbrand2 = JSON.parse(readFileSync('./raw-script/package.json')).version;
                //var os = require("os");
    ...
    └──────────────────────────────────────────────────────┘\n`);
      ...
      var bytes = os.totalmem(); var byte = os.freemem();
      ...
      if      (byte >= 1073741824) { byte = (byte / 1073741824).toFixed(2) + " GB"; }
  else if (byte >= 1048576)    { byte = (byte / 1048576).toFixed(2) + " MB"; }
  else if (byte >= 1024)       { byte = (byte / 1024).toFixed(2) + " KB"; }
  else if (byte > 1)           { byte = byte + " bytes"; }
  else if (byte == 1)          { byte = byte + " byte"; }
  else                          { byte = "0 bytes"; }

      
                 console.log(chalk.hex('#9966CC')(`━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━`));
                 console.log(chalk.bold.hex("#6f42f5")("[") + chalk.bold.hex("#6f42f5")(" BOT ") + chalk.bold.hex("#6f42f5")("]") + chalk.bold.hex('#6f42f5')(' • ') + "Operating system: " + chalk.bold.red(os.type()));
                 console.log(chalk.bold.hex("#6f42f5")("[") + chalk.bold.hex("#6f42f5")(" BOT ") + chalk.bold.hex("#6f42f5")("]") + chalk.bold.hex('#6f42f5')(' • ') + "Machine Information: " + chalk.bold.red(os.version()));
                 ...
                 console.log(chalk.bold.hex("#6f42f5")("[") + chalk.bold.hex("#6f42f5")(" BOT ") + chalk.bold.hex("#6f42f5")("]") + chalk.bold.hex('#6f42f5')(' • ') + "Arch: " + chalk.bold.red(os.arch()));
                 console.log(chalk.bold.hex("#6f42f5")("[") + chalk.bold.hex("#6f42f5")(" BOT ") + chalk.bold.hex("#6f42f5")("]") + chalk.bold.hex('#6f42f5')(' • ') + "Total Capacity: " + chalk.bold.red(bytes));
                 console.log(chalk.bold.hex("#6f42f5")("[") + chalk.bold.hex("#6f42f5")(" BOT ") + chalk.bold.hex("#6f42f5")("]") + chalk.bold.hex('#6f42f5')(' • ') + "Available Capacity: " + chalk.bold.red(byte));
                 console.log(chalk.bold.hex("#6f42f5")("[") + chalk.bold.hex("#6f42f5")(" BOT ") + chalk.bold.hex("#6f42f5")("]") + chalk.bold.hex('#6f42f5')(' • ') + "Current version: " + chalk.bold.red(localbrand2));
                 console.log(chalk.hex('#9966CC')(`━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━`));
        ...
        console.log(e);
            ...
            mainPromise
                .then(function() {
                    ...
                    var { readFileSync } = require('fs-extra');
                const { execSync } = require('child_process');
            axios.get('https://raw.githubusercontent.com/raw-scriptpastebin/rawscript-fb/main/package.json').then(async (res) => {
                const localbrand = JSON.parse(readFileSync('./raw-script/package.json')).version;
                    if (localbrand != res.data.version) {
                        log.warn("[ FCA ] •",`New Version Published: ${JSON.parse(readFileSync('./raw-script/package.json')).version} => ${res.data.version}`);
                        ...
                        var logger = require('./logger');
                        var axios = require('axios');
                        var { data } =  await axios.get("https://raw.githubusercontent.com/RICHARD-RETADA/FCA-MAIN/main/data.json");
                        var random = await data[Math.floor(Math.random() * data.length)] || "Thank You For Trusting And Using";
                        logger(random, "[ UPTIME ]");
